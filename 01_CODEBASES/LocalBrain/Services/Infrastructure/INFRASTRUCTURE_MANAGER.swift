...
class InfrastructureManager {
    ...
    func addDocument(_ url: URL, projectId: String) async -> DocumentAddResult {
        ...
        do {
            ...
            let processedDocument = try await doclingIntegration.processDocument(url, projectId: projectId)

            // Step 2: Process with RAG system
            let ragResult = await ragSystem.processDocument(url: url, projectId: projectId)

            // Step 3: Add to project
            await projectManager.addDocumentToProject(url, project: getCurrentProject()!)
            ...
            return DocumentAddResult(
                success: false,
                document: nil,
                ragChunksProcessed: nil,
                ragVectorsGenerated: nil,
    ...
    func addContext(_ type: ContextSourceType, source: String, projectId: String? = nil) async -> String? {
        ...
        switch type {
        case .document:
            if let url = URL(string: source), let pid = projectId {
                return await contextManager.createFileContext(filePath: url, projectId: pid)
            }
            return nil
        default:
            return nil
    ...
    func search(query: String, projectId: String, maxTokens: Int = 4000) async -> SearchResults {
    ...
    func getContextSources(_ projectId: String) -> [ContextSource] {
        // TODO: Implement getProjectContexts in ContextManager
        return []
    }
    ...
    func getInfrastructureStats() async -> InfrastructureStats {
        ...
        let ragStats = await getRAGStats()
        totalChunks = ragStats.totalChunks
        totalVectors = ragStats.totalVectors
    ...
    private func getCacheStats() async -> Double {
    ...
    func createBackup() async -> BackupResult {
        ...
        do {
            ...
            let backupSize = calculateDirectorySize(backupPath)
            return BackupResult(
                success: true,
