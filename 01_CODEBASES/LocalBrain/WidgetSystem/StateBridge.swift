...
class StateBridge: ObservableObject {
    ...
    private func setNestedValue(_ dictionary: inout [String: Any], path: String, value: Any) {
        ...
        if keys.count == 1 {
            dictionary[String(keys[0])] = value
            return
        }
        ...
        // Set the final value using key path
        var path = keys.map { String($0) }
        setValueAtPath(&dictionary, keys: path, value: value)
    }

    private func setValueAtPath(_ dict: inout [String: Any], keys: [String], value: Any) {
        ...
        if keys.count == 1 {
            ...
            setValueAtPath(&nested, keys: Array(keys.dropFirst()), value: value)
            dict[keys[0]] = nested
        }
    }
