...
export class RAGDatabase {
  ...
  // Document operations
  async storeDocument(document: Omit<SpecDocument, 'id' | 'indexedAt' | 'updatedAt'>): Promise<string> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async getDocumentByPath(filePath: string): Promise<SpecDocument | null> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async getAllDocuments(): Promise<SpecDocument[]> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  // Chunk operations
  async storeChunk(chunk: Omit<Chunk, 'embedding'>): Promise<string> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  // Search operations
  async searchChunks(query: string, limit: number = 10): Promise<any[]> {
    if (!this.db) throw new Error('Database not initialized');
