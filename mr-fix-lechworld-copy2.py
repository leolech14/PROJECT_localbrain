...
class SelfHealingCodeManager:
    """
    Automatically enforces 88-char width and updates zone metadata on every script run
    SELF-HEALING FEATURES:
    1. Detects lines exceeding MAX_WIDTH → auto-wraps them
    2. Scans for zone markers → calculates accurate LOC counts
    3. Updates zone metadata → always correct line numbers
    4. Validates ASCII art → ensures perfect 88-char alignment
    5. Repairs broken markers → standardizes format
    6. Closes open box structures → all boxes have right walls
    """
...
class MrFixMyProjectPlease:
    ...
    # ╚════════════════════════════════════════════════════════════════════════════════════╝
    PURPOSE: Universal project analysis engine that adapts to any scale and complexity
    ARCHITECTURE:
    ────────────────────────────────────────────────────────────────────────────────────
    • Adaptive Strategy: Selects analysis approach based on project characteristics
    ...
    4. ADVANCED ANALYSIS: Duplicates, naming conventions, tech stack, risks
    OUTPUT FORMATS:
    ────────────────────────────────────────────────────────────────────────────────────
    • Interactive HTML report with bilingual support (EN/PT)
    ...
    def __init__(self, project_path: str = "."):
        ...
        PURPOSE: Set up all analysis components and configuration parameters
        PARAMETERS:
            project_path (str): Root directory to analyze (default: current directory)
        CONFIGURES:
            • Performance limits (time, memory, sampling rates)
            • Analysis state tracking and result storage
            • Translation system for bilingual output
            • Scoring weights for project health assessment
        """
        # ╔════════════════════════════════════════════════════════════════════════════════════╗
        # ║ [!] CORE CONFIGURATION                                                             ║
        # ╚════════════════════════════════════════════════════════════════════════════════════╝
...
Keep descriptions CONCISE and PURPOSE-FOCUSED."""
    ...
    def _generate_dependency_map_html(self) -> str:
        """Generate INTERACTIVE dependency map with Mermaid diagram visualization"""
        try:
            # Get dependency analysis results
            dependency_analysis = self.generate_dependency_analysis()

            # Generate valid Mermaid diagram
            mermaid_diagram = self._generate_mermaid_diagram(dependency_analysis)
        ...
        <section class="card" id="section-dependency-map" style="border:1px solid var(--border);border-radius:8px;padding:20px">
          <summary style="cursor:pointer;font-size:18px;font-weight:600;margin-bottom:16px;user-select:none" data-en="🦋 Codebase Inter-Dependency Map - Click to expand" data-pt="🦋 Mapa de Dependências Inter-Código - Clique para expandir">🦋 Codebase Inter-Dependency Map</summary>

          <div class="dependency-stats" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:20px;">
            <div class="stat-card" style="background:var(--surface-2);padding:16px;border-radius:8px;text-align:center;">
              <div style="font-size:1.5rem;font-weight:700;color:var(--accent);margin-bottom:8px;">{stats.get('total_files', 0)}</div>
              <div style="color:var(--muted);font-size:0.9rem;">Total Files</div>
            </div>
            <div class="stat-card" style="background:var(--surface-2);padding:16px;border-radius:8px;text-align:center;">
              <div style="font-size:1.5rem;font-weight:700;color:var(--warning);margin-bottom:8px;">{stats.get('direct_dependencies', 0)}</div>
              <div style="color:var(--muted);font-size:0.9rem;">Direct Dependencies</div>
            </div>
            <div class="stat-card" style="background:var(--surface-2);padding:16px;border-radius:8px;text-align:center;">
              <div style="font-size:1.5rem;font-weight:700;color:var(--success);margin-bottom:8px;">{stats.get('indirect_dependencies', 0)}</div>
              <div style="color:var(--muted);font-size:0.9rem;">Indirect Dependencies</div>
            </div>
            <div class="stat-card" style="background:var(--surface-2);padding:16px;border-radius:8px;text-align:center;">
              <div style="font-size:1.5rem;font-weight:700;color:var(--danger);margin-bottom:8px;">{stats.get('risk_level', 'LOW')}</div>
              <div style="color:var(--muted);font-size:0.9rem;">Risk Level</div>
            </div>
          </div>

          <div class="mermaid-container" style="background:var(--surface-2);border:1px solid var(--border);border-radius:8px;padding:20px;min-height:400px;overflow-x:auto;">
            <div class="mermaid" id="dependency-mermaid">
              {mermaid_diagram}
            </div>
          </div>

          <div class="dependency-actions" style="margin-top:20px;display:flex;gap:12px;flex-wrap:wrap;">
            <button onclick="copyMermaidCode()" style="background:var(--accent);color:white;border:none;padding:12px 24px;border-radius:8px;cursor:pointer;font-weight:600;transition:all 0.2s;">
              📋 Copy Mermaid Code
            </button>
            <button onclick="exportDependencyMap()" style="background:var(--warning);color:white;border:none;padding:12px 24px;border-radius:8px;cursor:pointer;font-weight:600;transition:all 0.2s;">
              💾 Export Analysis
            </button>
            <button onclick="regenerateDependencyMap()" style="background:var(--muted);color:white;border:none;padding:12px 24px;border-radius:8px;cursor:pointer;font-weight:600;transition:all 0.2s;">
              🔄 Regenerate Map
            </button>
          </div>

          <div class="dependency-recommendations" style="margin-top:20px;">
            <h4 style="margin-bottom:12px;color:var(--text);">🎯 Dependency Recommendations:</h4>
            <div class="recommendations-list" style="background:var(--surface-2);border:1px solid var(--border);border-radius:8px;padding:16px;">
              <div class="recommendation" style="margin-bottom:8px;color:var(--text-secondary);padding:8px;background:var(--surface);border-radius:6px;border-left:3px solid var(--accent);">
                <strong>PRIORITY:</strong> Update affected components when adding new features to prevent breaking changes.
              </div>
              <div class="recommendation" style="margin-bottom:8px;color:var(--text-secondary);padding:8px;background:var(--surface);border-radius:6px;border-left:3px solid var(--warning);">
                <strong>MEDIUM:</strong> Review circular dependencies that may impact maintainability.
              </div>
              <div class="recommendation" style="color:var(--text-secondary);padding:8px;background:var(--surface);border-radius:6px;border-left:3px solid var(--success);">
                <strong>LOW:</strong> Consider adding integration tests for critical dependency paths.
              </div>
            </div>
          </div>
        </section>
        """
            ...
            return f"""
        <section class="card" id="section-dependency-map" style="border:1px solid var(--border);border-radius:8px;padding:20px;">
          <h2>🦋 Codebase Inter-Dependency Map</h2>
    ...
    def _generate_mermaid_diagram(self, dependency_analysis: dict) -> str:
        """Generate ALWAYS VALID Mermaid diagram syntax from dependency analysis"""
        try:
            # Extract nodes and relationships from analysis
            direct_deps = dependency_analysis.get("direct_dependencies", [])
            indirect_deps = dependency_analysis.get("indirect_dependencies", [])

            # Build always-valid Mermaid diagram
            diagram_lines = [
                "graph TD",
                "    %% Codebase Dependency Map - Always Valid Syntax",
                "    %% Generated: " + str(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")),
                "",
                "    %% Main Feature Node",
                "    FEATURE[\"New Feature<br/>\" + \"Click to analyze dependencies\"]",
                ""
            ]

            # Add direct dependencies
            for i, dep in enumerate(direct_deps[:8]):  # Limit for readability
                dep_id = f"DEP{i+1}"
                dep_label = dep.get("name", f"Component {i+1}")
                dep_type = dep.get("type", "module")

                # Choose appropriate icon based on type
                icons = {"class": "🏗️", "function": "⚡", "module": "📦", "config": "⚙️"}
                icon = icons.get(dep_type, "📄")

                diagram_lines.extend([
                    f"    {dep_id}[\"{icon} {dep_label}<br/><small>{dep_type}</small>\"]",
                    f"    FEATURE --> {dep_id}",
                    ""
                ])

            # Add indirect dependencies
            for i, dep in enumerate(indirect_deps[:5]):  # Limit for readability
                dep_id = f"IND{i+1}"
                dep_label = dep.get("name", f"Indirect {i+1}")

                diagram_lines.extend([
                    f"    {dep_id}[\"📎 {dep_label}<br/><small>indirect</small>\"]:::indirect",
                    f"    DEP1 --> {dep_id}",
                    ""
                ])
            ...
            diagram_lines.extend([
                ...
                "    classDef indirect fill:#6b7280,stroke:#374151,color:white",
                "    classDef feature fill:#dc2626,stroke:#991b1b,color:white",
                "",
                "    class FEATURE feature",
                "    class DEP1,DEP2,DEP3,DEP4,DEP5 primary",
                "    class DEP6,DEP7,DEP8 secondary",
                "    class IND1,IND2,IND3,IND4,IND5 indirect",
                "",
                "    %% Legend",
            ...
            return """graph TD
    %% Error Fallback - Always Valid
    A[Dependency Analysis Error]
    B["Error: """ + str(e)[:50] + """..."]
    A --> B
...
"""
    ...
    def generate_dependency_analysis(self) -> dict:
        """Generate dependency analysis results"""
        try:
            # Analyze the current script structure
            ...
            # Count lines, classes, and functions
            with open(__file__, 'r', encoding='utf-8') as f:
                content = f.read()
                file_stats["lines"] = len(content.splitlines())
                file_stats["classes"] = len(re.findall(r'^class\s+\w+', content, re.MULTILINE))
                file_stats["functions"] = len(re.findall(r'^def\s+\w+', content, re.MULTILINE))

            # Simulate dependency analysis
            direct_deps = [
                {"name": "os", "type": "module", "strength": "strong"},
                {"name": "re", "type": "module", "strength": "strong"},
                {"name": "json", "type": "module", "strength": "medium"},
                {"name": "datetime", "type": "module", "strength": "medium"},
                {"name": "sys", "type": "module", "strength": "strong"},
                {"name": "pathlib", "type": "module", "strength": "medium"},
                {"name": "subprocess", "type": "module", "strength": "medium"},
                {"name": "openai", "type": "external", "strength": "weak"}
            ]

            indirect_deps = [
                {"name": "typing", "type": "module", "strength": "weak"},
                {"name": "collections", "type": "module", "strength": "weak"},
                {"name": "itertools", "type": "module", "strength": "weak"},
                {"name": "statistics", "type": "module", "strength": "weak"},
                {"name": "hashlib", "type": "module", "strength": "weak"}
            ]

            # Calculate risk level based on complexity
            risk_factors = {
                "high_complexity": file_stats["lines"] > 5000,
                "many_dependencies": len(direct_deps) > 10,
                "external_apis": any(dep["type"] == "external" for dep in direct_deps)
            }
            ...
            # Feature node (the script itself)
            feature_node = {
                "name": script_path,
                "type": "comprehensive_script",
                "description": "MEGALITHIC PROJECT ANALYSIS SYSTEM",
                "complexity": "HIGH",
                "impact": "CRITICAL"
            }

            # Critical files (high impact components)
            critical_files = [
                script_path,
                "requirements.txt",
                ".env.example",
            ...
            statistics = {
                ...
                "indirect_dependencies": len(indirect_deps),
                "critical_files": len(critical_files),
                "risk_level": risk_level,
                "complexity_score": risk_score,
                "lines_of_code": file_stats["lines"],
                "estimated_impact": "HIGH" if risk_score >= 2 else "MEDIUM"
            }
            ...
            return {
                ...
                "risk_assessment": {
                    ...
                    "recommendation": "REVIEW" if risk_score >= 2 else "PROCEED"
                },
                "analysis_metadata": {
                    "timestamp": datetime.datetime.now().isoformat(),
                    "analyzer": "MEGALITHIC_DEPENDENCY_ANALYZER",
                    "version": "1.0.0"
                }
            }
            ...
            # Fallback analysis
            return {
                "feature_node": {"name": "unknown", "type": "script"},
                "direct_dependencies": [],
...
def generate_architecture_map(script_path: str = __file__) -> str:
    ...
    def _generate_mermaid_diagram(self, dependency_analysis: dict) -> str:
        ...
        try:
            # Extract nodes and relationships from analysis
            feature_node = dependency_analysis.get("feature_node", {})
            direct_deps = dependency_analysis.get("direct_dependencies", [])
            indirect_deps = dependency_analysis.get("indirect_dependencies", [])
            ...
            # Add direct dependencies
            if direct_deps:
                diagram_lines.append("    %% Direct Dependencies")
                for i, dep in enumerate(direct_deps[:10]):  # Limit to 10 for readability
                    dep_name = dep.get("name", f"Component_{i+1}").replace('"', '\\"')
                    dep_type = dep.get("type", "File").upper()
                    diagram_lines.append(f'    COMP{i+1}[\"{dep_name}<br/>{dep_type}\"]')
                    diagram_lines.append(f"    FEATURE --> COMP{i+1}")
                diagram_lines.append("")

            # Add indirect dependencies
            if indirect_deps:
                diagram_lines.append("    %% Indirect Dependencies")
                for i, dep in enumerate(indirect_deps[:5]):  # Limit to 5 for readability
                    dep_name = dep.get("name", f"Indirect_{i+1}").replace('"', '\\"')
                    dep_type = dep.get("type", "File").upper()
                    diagram_lines.append(f'    INDIRECT{i+1}[\"{dep_name}<br/>{dep_type}\"]')
                    diagram_lines.append(f"    FEATURE -.-> INDIRECT{i+1}")
                diagram_lines.append("")

            # Add styling definitions
            diagram_lines.extend([
                "    %% Styling",
                "    classDef feature fill:#ff9999,stroke:#ff6666,stroke-width:3px",
                "    classDef direct fill:#99ccff,stroke:#66aaff,stroke-width:2px",
                "    classDef indirect fill:#ccffcc,stroke:#66cc66,stroke-width:1px",
                "    class FEATURE feature",
                "    class COMP1,COMP2,COMP3,COMP4,COMP5,COMP6,COMP7,COMP8,COMP9,COMP10 direct",
                "    class INDIRECT1,INDIRECT2,INDIRECT3,INDIRECT4,INDIRECT5 indirect"
            ])

            return "\n".join(diagram_lines)

        except Exception as e:
            # Fallback to basic diagram if analysis fails
            return f"""graph TD
    %% Basic Dependency Map
    FEATURE["Feature Analysis<br/>Analysis Failed"]
    ERROR["Error: {str(e)}"]
    FEATURE --> ERROR
    ...
    def generate_dependency_analysis(self) -> dict:
        ...
        try:
            ...
            # Find Python imports and class definitions
            imports = re.findall(r'^(?:from|import)\s+([^\s#]+)', script_content, re.MULTILINE)
            classes = re.findall(r'^class\s+(\w+)', script_content, re.MULTILINE)
            functions = re.findall(r'^def\s+(\w+)', script_content, re.MULTILINE)

            # Simulate dependency relationships
            direct_deps = []
            indirect_deps = []

            # Add Python standard library dependencies
            ...
            return {
                ...
                "indirect_dependencies": indirect_deps,
                "statistics": {
                    "total_files": len(imports) + len(classes) + len(functions),
                    "direct_dependencies": len(direct_deps),
                    "indirect_dependencies": len(indirect_deps),
                    "risk_level": "LOW" if len(direct_deps) < 3 else "MEDIUM"
                }
            }
        except Exception as e:
            return {
                "feature_node": {"name": "Unknown", "path": "Unknown", "type": "Unknown"},
                "direct_dependencies": [],
                "indirect_dependencies": [],
                "statistics": {"total_files": 0, "direct_dependencies": 0, "indirect_dependencies": 0, "risk_level": "UNKNOWN"},
                "error": str(e)
            }
    md.append("")
    md.append("---")
    md.append("")
    md.append("## 🎯 CRITICAL SECTIONS (HARDCODED)")
    md.append("")
    md.append("### 1. File Counting Accuracy")
    md.append("```bash")
    md.append("# Auto-detection logic")
    md.append("sed -n '996,1013p' mr-fix-my-project-please.py")
    md.append("")
    md.append("# Conditional sampling")
    md.append("sed -n '1098,1110p' mr-fix-my-project-please.py")
    md.append("")
    md.append("# COMPREHENSIVE_MODE flag")
    md.append("sed -n '245,246p' mr-fix-my-project-please.py")
    md.append("```")
    md.append("")
    md.append("### 2. Output Standardization")
    md.append("```bash")
    md.append("# Enhanced results dict")
    md.append("sed -n '4268,4279p' mr-fix-my-project-please.py")
    md.append("")
    md.append("# Remediation embedding")
    md.append("sed -n '5393,5405p' mr-fix-my-project-please.py")
    md.append("```")
    md.append("")
    md.append("### 3. HTML Generation")
    md.append("```bash")
    md.append("# Main HTML generator")
    md.append("sed -n '2705,2881p' mr-fix-my-project-please.py")
    md.append("")
    md.append("# Markdown parser (GPT-5 formatting)")
    md.append("sed -n '3200,3371p' mr-fix-my-project-please.py")
    md.append("```")
    md.append("")
    md.append("### 4. GPT-5 Integration")
    md.append("```bash")
    md.append("# API integration & parsing")
    md.append("sed -n '5023,5177p' mr-fix-my-project-please.py")
    md.append("```")
    md.append("")
    md.append("---")
    md.append("")
    ...
    md.append("")
    md.append("### Find All Methods")
    md.append("```bash")
    md.append("grep -n '^    def ' mr-fix-my-project-please.py")
    md.append("```")
    md.append("")
    md.append("### Find Section Generators")
    md.append("```bash")
    md.append("grep -n 'def _generate_.*_html' mr-fix-my-project-please.py")
    md.append("```")
    md.append("")
    md.append("### Find HTML Headings")
    md.append("```bash")
    md.append("grep -n '<h[1-6]' mr-fix-my-project-please.py")
    md.append("```")
    md.append("")
    md.append("---")
    md.append("")
    ...
    md.append("")
    md.append(f"- **Total Lines:** {total_lines:,}")
    md.append(f"- **Classes:** {len(class_items)}")
    md.append(f"- **Methods:** {len([s for s in sections if s['type'] == 'METHOD'])}")
    md.append(
        f"- **Functions:** {len([s for s in sections if s['type'] == 'FUNCTION'])}"
    )
    md.append("")
    md.append("---")
    md.append("")
    md.append("**🔄 Regenerate this map:**")
    md.append("```bash")
    md.append("python3 mr-fix-my-project-please.py --map")
    md.append("```")
    md.append("")
    md.append(
        "**🎯 This map is generated from code structure and hardcoded critical sections.**"
    )
    md.append("**Critical sections are manually maintained for surgical precision.**")
    return "\n".join(md)
# ╔════════════════════════════════════════════════════════════════════════════════════╗
# ║ MAIN ENTRY POINT                                                                   ║
# ╚════════════════════════════════════════════════════════════════════════════════════╝
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--map":
        print("🗺️ Generating architecture map...")
        map_content = generate_architecture_map()
        output_path = Path(__file__).parent / "SCRIPT_ARCHITECTURE_MAP.md"
        output_path.write_text(map_content, encoding="utf-8")
        print(f"✅ Architecture map saved: {output_path}")
        print(f"   Lines analyzed: {len(Path(__file__).read_text().split(chr(10)))}")
        return
    # 🔧 SELF-HEALING SYSTEM (RUNS FIRST - ALWAYS)
    print("🔧 Self-Healing System: Validating script structure...")
    healer = SelfHealingCodeManager(Path(__file__))
    healing_applied = healer.analyze_and_heal()
    if healing_applied:
        print("   🩹 Script was healed - width/zones corrected")
        print(healer.get_zone_report())
    else:
        print("   ✅ Script structure validated - no healing needed")
    # 🗺️ AUTO-GENERATE ARCHITECTURE MAP (ALWAYS)
    print("\n🗺️ Auto-generating architecture map...")
    map_content = generate_architecture_map()
    output_path = Path(__file__).parent / "SCRIPT_ARCHITECTURE_MAP.md"
    output_path.write_text(map_content, encoding="utf-8")
    ...
    try:
        ...
        # Generate autonomous remediation instructions for Claude Code agent
        current_score = results.get("score", 50.0)
        remediation_plan_data = None
        if current_score < 85.0:
            print(f"\n🤖 Generating autonomous remediation instructions...")
            remediation_plan = analyzer.generate_agent_remediation_instructions(
                current_score=current_score, target_score=85.0
            )
            # Also store structured remediation data in results for JSON output
            remediation_plan_data = {
                "current_score": current_score,
                "target_score": 85.0,
                "score_gap": 85.0 - current_score,
                "markdown_instructions": remediation_plan,
                "generated": True,
            }
            results["remediation_plan"] = remediation_plan_data
            # Re-write JSON with remediation plan included
            with open("maximum_extraction_results.json", "w", encoding="utf-8") as f:
                json.dump(results, f, indent=2, default=str)
            # Write to markdown file
            instructions_path = Path(project_path) / "CLAUDE_AGENT_INSTRUCTIONS.md"
            with open(instructions_path, "w", encoding="utf-8") as f:
                f.write(remediation_plan)
            print(f"✅ Agent instructions saved to: {instructions_path}")
            print(f"   📋 Score: {current_score:.1f}/100 → Target: 85.0/100")
