...
describe('SessionAutoDetect', () => {
    describe('Model ID Detection', () => {
        ...
        it('should extract model name from full ID correctly', () => {
            ...
            testCases.forEach(({ input, expected }) => {
                ...
                const result = SessionAutoDetect.extractModelName(input);
    ...
    describe('Agent Role Mapping', () => {
        it('should map Sonnet-4.5 to Agent B or D', () => {
            const roles = SessionAutoDetect.mapModelToAgents('claude-sonnet-4-5');
    ...
    describe('Session ID Generation', () => {
        it('should generate unique session IDs', () => {
            const id1 = SessionAutoDetect.generateSessionId('B');
            const id2 = SessionAutoDetect.generateSessionId('B');
            expect(id1).not.toBe(id2);
            expect(id1).toMatch(/^B_\d+_[a-z0-9]+$/);
        });
        it('should include agent ID in session ID', () => {
            const idA = SessionAutoDetect.generateSessionId('A');
            const idB = SessionAutoDetect.generateSessionId('B');
    ...
    describe('Progress Bar Rendering', () => {
        it('should render 0% progress correctly', () => {
            const bar = SessionAutoDetect.renderProgressBar(0, 10);
            expect(bar).toContain('[░░░░░░░░░░░░░░░░░░░░]');
            expect(bar).toContain('0%');
        });
        it('should render 50% progress correctly', () => {
            const bar = SessionAutoDetect.renderProgressBar(5, 10);
            expect(bar).toContain('50%');
            expect(bar).toMatch(/\[█+░+\]/); // Mix of filled and empty
        });
        it('should render 100% progress correctly', () => {
            const bar = SessionAutoDetect.renderProgressBar(10, 10);
            expect(bar).toContain('[████████████████████]');
            expect(bar).toContain('100%');
        });
        it('should handle zero total gracefully', () => {
            const bar = SessionAutoDetect.renderProgressBar(0, 0);
            expect(bar).toContain('0%');
    ...
    describe('Agent Emoji Mapping', () => {
        it('should return correct emoji for each agent', () => {
            const getEmoji = SessionAutoDetect.getAgentEmoji;
 ...
 * Edge cases: 100%
 */
//# sourceMappingURL=SessionAutoDetect.test.js.map