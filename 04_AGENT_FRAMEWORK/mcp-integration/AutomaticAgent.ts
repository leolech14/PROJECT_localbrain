...
import { SessionAutoDetect, AgentIdentity } from './SessionAutoDetect.js';
import { NaturalLanguageRouter, TaskAction } from './NaturalLanguageRouter.js';
...
export class AutomaticAgent {
   ...
   */
  private constructor(identity: AgentIdentity) {
   ...
   */
  async processPrompt(prompt: string): Promise<TaskAction | null> {
    if (!this.initialized) {
   ...
   */
  async refreshStatus(): Promise<AgentIdentity> {
    try {
   ...
   */
  async isTaskAction(prompt: string): Promise<boolean> {
 ...
 * ```
 */
export async function quickStart(): Promise<AgentIdentity> {
