...
export interface AgentIdentity {
  ...
  modelName: string;         // Short model name (e.g., claude-sonnet-4-5)
  agentId: 'A' | 'B' | 'C' | 'D' | 'E' | 'F';
  agentRole: string;         // Human-readable role (e.g., "Design System Specialist")
  emoji: string;             // Agent emoji for visual identification
  taskStatus: {
    total: number;
    completed: number;
    available: number;
  ...
  capabilities: string[];    // Agent capabilities/specializations
  sessionId: string;         // Unique session identifier
}

/**
 * Model to Agent Mapping - Maps Claude models to agent roles
 */
interface AgentRole {
  agentId: 'A' | 'B' | 'C' | 'D' | 'E' | 'F';
  name: string;
...
export class SessionAutoDetect {
   ...
   */
  private static mapModelToAgents(modelName: string): AgentRole[] {
    const modelMap: Record<string, AgentRole[]> = {
      'glm-4-6': [
        {
          agentId: 'A',
   ...
   */
  private static async selectActiveRole(possibleRoles: AgentRole[]): Promise<AgentRole> {
   ...
   */
  private static generateSessionId(agentId: string): string {
 ...
 * ```
 */
export async function startSession(): Promise<AgentIdentity> {
